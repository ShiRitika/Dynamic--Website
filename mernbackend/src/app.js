...
//middleware
app.use("/css", express.static(path.join(__dirname, "../node_modules/bootstrap/dist/css")));//we had passed route to /css
...
app.get("/",(req, res)=> { //rounting with respond
...
app.post("/registration", async(req, res) => { //wehen data get enter submit button click send data to contact page
    try {
        ...
        res.send(400).send(error); //otherwise error
    }
});

app.post("/contact", async(req, res) => { //wehen data get enter submit button click send data to contact page
    try {
